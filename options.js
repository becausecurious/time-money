...
//  chrome.storage.sync.remove('percentOfWordsToReplace');
  ...
  var usdPerHour = document.getElementById('usdPerHour').value
  	if (usdPerHour.includes('/',1) == true) {
  		pSpD1earnAvg = usdPerHour.split('/');
  		usdPerHour = Number(pSpD1earnAvg[0]) / Number(pSpD1earnAvg[1]); 		
  	}
  	else {
  	   usdPerHour = parseFloat(Number(usdPerHour), 10);
    }
